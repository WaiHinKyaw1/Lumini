...
const SpeechMaster: React.FC<SpeechMasterProps> = ({ onSpendCredits, onSaveResult, history, onDelete }) => {
  ...
  const handleHistorySelect = (item: GenerationResult) => {
    if (item.content) setText(item.content);
    if (item.url) setAudioUrl(item.url);
    if (item.metadata?.voice) setSelectedVoice(item.metadata.voice);
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      onSaveResult({
        type: ContentType.SPEECH,
        prompt: `TTS: ${text.slice(0, 40)}...`,
        url: blobUrl,
        content: text,
  ...
  return (
    <div className="max-w-4xl mx-auto pb-20 animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h1 className="text-xl font-bold text-slate-900 dark:text-white uppercase tracking-tighter italic">Speech Master Synthesis</h1>
          <p className="text-slate-500 dark:text-zinc-400 text-[10px] font-black uppercase tracking-widest">Real-time Audio Database Engine</p>
        </div>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        <div className="lg:col-span-8 space-y-4">
          <div className="glass p-6 rounded-[2.5rem] border border-white/5 space-y-5 shadow-2xl relative overflow-hidden">
            <div className="absolute -top-10 -right-10 w-32 h-32 bg-indigo-500/10 rounded-full blur-3xl"></div>
            <div>
              <label className="block text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 dark:text-zinc-500 mb-3">Input Neural Script</label>
              <textarea
                value={text}
                onChange={(e) => setText(e.target.value)}
                placeholder="Enter text for broadcast-quality synthesis..."
                className="w-full h-56 bg-slate-50 dark:bg-black/30 border border-slate-200 dark:border-white/10 rounded-[2rem] p-6 text-sm text-slate-900 dark:text-white focus:ring-2 focus:ring-indigo-500 outline-none transition-all resize-none font-medium leading-[2]"
              />
              <div className="mt-3 flex justify-between items-center px-2">
                <span className="text-[10px] font-black text-slate-400 dark:text-zinc-600 uppercase tracking-widest">{text.length} Chars</span>
                <button onClick={() => setText('')} className="text-[10px] font-black text-rose-500 uppercase tracking-widest hover:underline transition-all">Format Reset</button>
              </div>
            </div>
            ...
            <button
              ...
              className={`w-full py-4 rounded-2xl text-[11px] font-black uppercase tracking-[0.25em] transition-all shadow-2xl active:scale-[0.98] ${
        ...
        <div className="lg:col-span-4 space-y-4">
          <div className="glass p-5 rounded-[2rem] border border-white/5 space-y-4 shadow-xl">
            <h3 className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-400 dark:text-zinc-500 px-2">Talent Roster</h3>
            <div className="space-y-1.5 max-h-[400px] overflow-y-auto custom-scrollbar pr-1">
              {voices.map((voice) => (
                ...
                >
                  <span className="text-2xl group-hover:scale-110 transition-transform">{voice.icon}</span>
                  <div>
                    <h4 className="text-[10px] font-black uppercase tracking-widest">{voice.name}</h4>
                    <p className={`text-[9px] font-bold mt-0.5 ${selectedVoice === voice.id ? 'text-indigo-100' : 'text-slate-400'}`}>{voice.desc}</p>
                  </div>
                </button>
              ))}
            </div>
          ...
          {audioUrl && (
            <div className="glass p-6 rounded-[2rem] border border-emerald-500/30 bg-emerald-500/5 animate-in zoom-in-95 duration-500 shadow-2xl">
              <div className="flex items-center gap-6">
                ...
                <div className="flex-1">
                  <h4 className="text-[11px] font-black text-slate-900 dark:text-white uppercase tracking-widest mb-1 italic">✓ Master Locked</h4>
                  <a
                    href={audioUrl}
                    download="lumina_real_data_export.wav"
                    className="flex items-center gap-2 text-[10px] font-black text-indigo-500 dark:text-indigo-400 uppercase tracking-widest hover:underline"
                  >
      ...
      {error && (
        <div className="mt-8 p-6 bg-rose-500/10 border border-rose-500/20 text-rose-500 text-[10px] font-black text-center rounded-[2rem] animate-in shake duration-500 uppercase tracking-widest">
